...
app.post("/scrape", async (req, res) => {
  try {
    ...
    // Detectar si es puromarketing.com
...
app.post("/scrape-single", async (req, res) => {
  try {
    ...
    const $ = cheerio.load(response.data);
    const isPuroMarketing = url.includes("puromarketing.com");

    const selectores = isPuroMarketing
      ? {
          titulo: [
            ".item-title",
            "h1.entry-title",
          ...
          fecha: [".item-date", "time.entry-date"],
          autor: [".item-author", ".author-name"],
          imagen: [".item-image img", "article img"],
        }
      : {
          titulo: [
            "h1.entry-title",
